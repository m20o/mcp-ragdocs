...
export class OllamaProvider implements EmbeddingProvider {
  ...
  async generateEmbeddings(text: string): Promise<number[]> {
    try {
      logger.debug('Generating Ollama embeddings for text:', text.substring(0, 50) + '...');
      const response = await ollama.embeddings({
        model: this.model,
        prompt: text
      });
      logger.debug('Successfully generated Ollama embeddings with size:', response.embedding.length);
...
export class OpenAIProvider implements EmbeddingProvider {
  ...
  async generateEmbeddings(text: string): Promise<number[]> {
    try {
      logger.debug('Generating OpenAI embeddings for text:', text.substring(0, 50) + '...');
      const response = await this.client.embeddings.create({
        model: this.model,
        input: text,
      });
      const embedding = response.data[0].embedding;
      logger.debug('Successfully generated OpenAI embeddings with size:', embedding.length);
      return embedding;
    } catch (error) {
